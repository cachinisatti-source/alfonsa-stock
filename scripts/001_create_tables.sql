...
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON stock_controls;
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON stock_items;

-- Crear políticas de seguridad (permitir acceso a todos los usuarios)
CREATE POLICY "Allow all operations for authenticated users" ON stock_controls
    FOR ALL USING (true);
